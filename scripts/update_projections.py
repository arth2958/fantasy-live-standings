...
def main():
    ...
    player_proj=defaultdict(float); player_actual=defaultdict(float); player_left=defaultdict(int); player_paired=defaultdict(int); player_in_progress=defaultdict(int)
    ...
    for t in standings['teams']:
        ...
        fantasy.append({'owner':t['owner'],'name':t['name'],'actual_points':t['points'],'projected_points':t['points']+pending,'projected_round':sum(player_proj[h.casefold()] for h in roster),'left':sum(player_left[h.casefold()] for h in roster),'in_progress':sum(player_in_progress[h.casefold()] for h in roster),'paired':sum(player_paired[h.casefold()] for h in roster),'roster':[{'handle':h,'status':('pending' if player_left[h.casefold()] else ('won' if player_actual[h.casefold()] > 1 or (player_paired[h.casefold()] == 1 and player_actual[h.casefold()] == 1) else 'draw' if player_actual[h.casefold()] == player_paired[h.casefold()]/2 else 'lost') if player_paired[h.casefold()] else 'unpaired' if h.casefold() in current_roster else 'no_longer_rostered'),'rating':ratings.get(h.casefold(),{}).get('rating'),'expected_points':player_proj.get(h.casefold(),0),'actual_result':(player_actual.get(h.casefold()) if player_paired[h.casefold()] and not player_left[h.casefold()] else None),'round_points':player_actual.get(h.casefold(),0),'round_games':player_paired[h.casefold()]-player_left[h.casefold()],'game_url':(('https://lichess.org/'+live_game[h.casefold()]) if h.casefold() in live_game else ('https://lichess.org/'+recent_game[h.casefold()]) if h.casefold() in recent_game else None),'game_link_kind':('live' if h.casefold() in live_game else 'recent' if h.casefold() in recent_game else None),'season_points':next((p.get('points',0) for p in t['roster'] if p.get('handle','').casefold()==h.casefold()),0),'season_games':next((p.get('games',0) for p in t['roster'] if p.get('handle','').casefold()==h.casefold()),0)} for h in roster]})
